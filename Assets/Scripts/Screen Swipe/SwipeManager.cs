...
// See "MD_Reptile" 's post for source
...
public class SwipeManager : MonoBehaviour
{
    ...
    public System.Action<SwipeAction> onFrameSwipe;

    public static event Action onSwipeDetected;

    public static void InvokeOnSwipeDetected()
    {
        onSwipeDetected();
    }
    ...
    SwipeAction frameSwipeAction;

    private void Start()
    {
        currentSwipeAction = new SwipeAction();
        frameSwipeAction = new SwipeAction();
    }
    ...
    public void DetectSwipe()
    {
        ...
        if (touches.Count > 0)
        {
            ...
            {
                UpdateFrameSwipeAction(t);
                UpdateCurrentSwipeAction(t);
                if (!currentSwipeAction.longPress && currentSwipeAction.duration > longPressDuration
                ...
                if (onFrameSwipe != null)
                {
                    onFrameSwipe(frameSwipeAction);
                }
            }
    ...
    void UpdateFrameSwipeAction(Touch t)
    {
        frameSwipeAction.startPosition = currentSwipeAction.endPosition;
        frameSwipeAction.startTime = currentSwipeAction.endTime;

        frameSwipeAction.endPosition = new Vector2(t.position.x, t.position.y);
        frameSwipeAction.endTime = Time.time;
        frameSwipeAction.duration = frameSwipeAction.endTime - frameSwipeAction.startTime;

        var currentFrameSwipe = frameSwipeAction.endPosition - frameSwipeAction.startPosition;
        frameSwipeAction.rawDirection = currentFrameSwipe;
        frameSwipeAction.direction = GetSwipeDirection(currentFrameSwipe);
        frameSwipeAction.distance = Vector2.Distance(frameSwipeAction.startPosition, frameSwipeAction.endPosition);
        //if (currentSwipeAction.distance > currentSwipeAction.longestDistance) // If new distance is longer than previously longest
        //{
        //    currentSwipeAction.longestDistance = currentSwipeAction.distance; // Update longest distance
        //}
    ...
    SwipeDirection GetSwipeDirection(Vector2 direction)
    {
        ...
        //Debug.Log($"GetDirection -> direction = {direction.x}");
